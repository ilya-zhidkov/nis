...
public sealed class CoursesController(HttpClient client, IOptions<MoodleOptions> options) : BaseApiController
{
    ...
    public async Task<IActionResult> Index([FromHeader] string token)
    {
        ...
        var response = await client.GetAsync($"{url}/webservice/rest/server.php?wstoken={token}&moodlewsrestformat={format}&wsfunction=core_course_get_courses");
    ...
    {
        var (url, _, _, _, format) = _options;
        var response = await client.GetAsync($"{url}/webservice/rest/server.php?wstoken={token}&moodlewsrestformat={format}&wsfunction=mod_assign_get_assignments&courseids[0]={id}");
        var content = await response.Content.ReadAsStringAsync();
