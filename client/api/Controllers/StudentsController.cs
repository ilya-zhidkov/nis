...
public sealed class StudentsController(IOptions<MoodleOptions> options, HttpClient http) : BaseApiController
{
    ...
    public async Task<IActionResult> Index([FromHeader] string token)
    {
        ...
        var response = await http.GetAsync($"{url}/webservice/rest/server.php?wstoken={token}&moodlewsrestformat={format}&wsfunction=core_enrol_get_enrolled_users&courseid={courseId}");
    ...
    public async Task<IActionResult> Show([FromHeader] string token, string username)
    {
        ...
        var response = await http.GetAsync($"{url}/webservice/rest/server.php?wstoken={token}&moodlewsrestformat={format}&wsfunction=core_user_get_users_by_field&field={nameof(username)}&values[0]={username}");
        var content = await response.Content.ReadAsStringAsync();
